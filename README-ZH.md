...
`Tidis` 是 TiKV 的服务层, 旨在提供由 PingCAP 驱动的兼容 redis 协议的分布式存储服务。 Redis 协议已经实现了对多种数据类型（string/hash/list/set/sortedset）的支持。

为了更加强劲的性能和更低的延迟，同时为了增加更多诸如 Lua 脚本，TLS 连接，锁优化等重要功能的支持，我们设计了全新的 `Tidis` 并使用 `Rust` 语言进行了重写。

## 背景

Redis 是一个被很多公司和项目使用的支持多种数据结构的内存数据存储系统。虽然它在缓存和数据存储方面已经是一个优秀的解决方案，但它目前依然存在一些限制：

- 内存成本高，不适合存储大量的数据
- 不易于管理，且对大数据集的扩容不够友好
- 很多用例都依赖于协议和应用程序接口，并不追求极致的性能
- 不支持全局事务
- 没有数据持久化保证

我们相信，在低成本的存储介质基础上建立一个兼容 Redis 的分布式存储服务将会是一个好主意。更进一步说，有一些诸如 `ssdb`、`pika`、`kvrocks` 等等流行的产品正是基于这种想法建立的。然而，它们都是存算一体架构，这意味着建立数据副本、数据高可用、数据分片、数据扩展等基本的能力将会是非常复杂的。它不仅难以实现，而且运维起来也十分复杂。

考虑到这些复杂性，我们采取了不同的方案：在 [TiKV](https://tikv.org/) 的基础上建立了一个精简的计算服务层，即这是一个基于 `Raft` 共识算法的分布式存储系统。`TiKV` 是一个优秀的存储系统构建模块，它有很多功能，如高可用、数据分片、数据扩展、全局事务等等。基于此点，我们只需要专注于数据模型和计算，即服务层。

当然，我们并不是唯一一个拥有此类想法的人。`Titan` 和 `Tidis 1.0` 都是以类似的架构设计的，它们在早期时都很受欢迎，并在一定程度上发展了它们的社区。不幸的是，它们都有很多有用的功能没有实现。我们相信社区总是期待着看到功能更完整的替代项目。因此，我们决定接管 Tidis 1.0 留下的东西，重新设计和实现下一代 Tidis，并加入一些重要但缺失的功能，如 Lua 脚本、TLS/SSL、锁优化、一阶段提交、异步提交等等许多其他的改进。
...
得益于 `TiKV` 集群的全局事务机制, `Tidis` 可以轻松地支持全局事务. 使用 `MULTI/EXEC/DISCARD` 命令就像使用 `Redis Cluster` 一样，同时还不需要关心 `CROSSSLOT` 错误, 就像使用一个单实例的 `Redis` 一样。
...
`Tidis` 使用 `mlua` 库来转译 lua 脚本. 我们可以使用 `EVAL/EVALSHA` 来执行在全局事务支持下的 lua 脚本, 而不再需要关心 `CROSSSLOT` 错误。

Lua 脚本将会在一个新的事务上下文中运行，所以脚本中所有的读、写操作都是可以保证具备原子性的。

所有 lua 脚本的端到端测试用例位于 [test/test_lua.py](https://github.com/tidb-incubator/tidis/blob/master/test/test_lua.py) 。

## 异步删除

对于有成千上万个数据项的集合键，删除可能是一个耗时的操作，而启用异步删除配置则可以大大减少该类操作的耗时。

大键的删除任务将会在后台运行，同时立即响应用户，而无需等待后台任务的完成。如果旧版本已经以异步的方式删除，我们会维护同一个键的多个版本以保证并发安全。如果旧版本处于等待被删除的状态，则该键的新版本则会基于该键的最后一个版本单调递增，否则键的新版本将会从 0 开始。
...
你可以根据你的实际数据负载来调整参数。在下面的测试中，我们设置 `max_batch_wait_time` 为 10ms，`overload_threshold` 为 0，`max_batch_size` 不等。测试结果显示，当 `max_batch_size=20` 时，吞吐量增加了 50%，p999 延迟减少了 40%。

![](https://cdn.jsdelivr.net/gh/yongman/i@img/picgo/20220921111329.png)

## 性能

我们在 3 个 TiKV 节点，3 个 Tidis 节点，1 个 PD 节点和 1 个 TiDB 节点（用于垃圾回收）的集群拓扑上使用多个 `memtier-benchmark` 进程在不同的并发量下对集群进行了基准测试。基准测试结果显示，最大的`读`和`写`吞吐量分别为 `540k ops/s` 和 `125k op/s`。
...
我们还将基准结果与其他类 `Redis on TiKV` 项目（Titan 和 Tidis 1.0）进行了比较，结果显示，`Tidis` 比 Titan 和 Tidis 1.0 具有更高的写吞吐量。

![](https://cdn.jsdelivr.net/gh/yongman/i@img/picgo/20220921120700.png)

`Tidis` 的写入吞吐量几乎是 Titan 和 Tidis 1.0 的两倍，而且延迟总是最低的。

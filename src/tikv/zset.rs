                    ...
                    match txn.get_for_update(meta_key.clone()).await? {
                        Some(meta_value) => {
                            ...
                            for idx in 0..members.len() {
                                let data_key = KeyEncoder::new()
                                    .encode_txnkv_zset_data_key(&key, &members[idx]);
                                let new_score = scores[idx];
                                let score_key = KeyEncoder::new().encode_txnkv_zset_score_key(
                                    &key,
                                    new_score,
                                    &members[idx],
                                );
                                ...
                                if let Some(v) = exists {
                                    ...
                                    if (v && member_exists) || (!v && !member_exists) {
                                        ...
                                        let data_value = KeyEncoder::new()
                                            .encode_txnkv_zset_data_value(new_score);
                                    ...
                                    let data_value =
                                        KeyEncoder::new().encode_txnkv_zset_data_value(new_score);
                            ...
                            if added_count > 0 {
                                let new_meta_value = KeyEncoder::new()
                                    .encode_txnkv_zset_meta_value(ttl, size + added_count);
                        ...
                        None => {
                            ...
                            for idx in 0..members.len() {
                                ...
                                let member = members[idx].clone();
                                let score_key = KeyEncoder::new()
                                    .encode_txnkv_zset_score_key(&key, score, &member);
                                // add data key and score key
                                let data_value =
                                    KeyEncoder::new().encode_txnkv_zset_data_value(score);
                                txn.put(data_key, data_value).await?;
                            ...
                            let new_meta_value =
                                KeyEncoder::new().encode_txnkv_zset_meta_value(0, size);
                            txn.put(meta_key, new_meta_value).await?;
                            Ok(size)
                        }
                    }
    ...
    pub async fn do_async_txnkv_zcount(
        ...
        match ss.get(meta_key.to_owned()).await? {
            Some(meta_value) => {
                ...
                let start_key = KeyEncoder::new().encode_txnkv_zset_score_key_range_start(key, min);
                let end_key = KeyEncoder::new().encode_txnkv_zset_score_key_range_end(key, max);
                ...
                async move {
                    ...
                    match txn.get_for_update(meta_key.clone()).await? {
                        Some(meta_value) => {
                            ...
                            let mut resp = vec![];
                            if from_min {
                                let start_key = KeyEncoder::new()
                                    .encode_txnkv_zset_score_key_range_start(&key, 0);
                                let range = start_key..;
                                let from_range: BoundRange = range.into();
                                let iter = txn.scan(from_range, count.try_into().unwrap()).await?;

                                for kv in iter {
                                    let data_key = KeyEncoder::new().encode_txnkv_zset_data_key(
                                        &key,
                            ...
                            // update meta key
                            let new_meta_value = KeyEncoder::new()
                                .encode_txnkv_zset_meta_value(ttl, size - poped_count);
                            txn.put(meta_key, new_meta_value).await?;
            ...
            Some(meta_value) => {
                ...
                match ss.get(data_key).await? {
                    Some(data_value) => {
                        ...
                        let score = KeyDecoder::decode_key_zset_data_value(&data_value);
                        let score_key =
                            KeyEncoder::new().encode_txnkv_zset_score_key(key, score, member);

                        // scan from range start
                        let score_key_start =
                            KeyEncoder::new().encode_txnkv_zset_score_key_range_start(key, 0);
                    ...
                    match txn.get(meta_key.clone()).await? {
                        Some(meta_value) => {
                            ...
                            for member in members {
                                let data_key =
                                    KeyEncoder::new().encode_txnkv_zset_data_key(&key, &member);
                                ...
                                let score_key = KeyEncoder::new()
                                    .encode_txnkv_zset_score_key(&key, iscore, &member);
                                txn.delete(data_key).await?;
                            ...
                            // update meta key
                            let new_meta_value = KeyEncoder::new()
                                .encode_txnkv_zset_meta_value(ttl, size - removed_count);
                            txn.put(meta_key, new_meta_value).await?;
                            Ok(removed_count)
                        }
                        None => Ok(0),
                ...
                async move {
                    ...
                    match txn.get_for_update(meta_key.clone()).await? {
                        Some(meta_value) => {
                            ...
                            let score_key_start = KeyEncoder::new()
                                .encode_txnkv_zset_score_key_range_start(&key, min);
                            let score_key_end =
                                KeyEncoder::new().encode_txnkv_zset_score_key_range_end(&key, max);
                            ...
                            for kv in iter {
                                ...
                                let data_key = KeyEncoder::new().encode_txnkv_zset_data_key(
                                    &key,
                                    &String::from_utf8_lossy(&score_value),
                                );
                                txn.delete(data_key).await?;
                                txn.delete(kv.0).await?;
                                removed_count += 1;
